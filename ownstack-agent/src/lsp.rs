...
use lsp_types::{
    ...
    ClientCapabilities, Diagnostic, DocumentSymbolResponse, GotoDefinitionResponse,
    ...
    TextDocumentPositionParams, Url, WindowClientCapabilities,
};
...
pub struct LspClient {
    ...
    pending_requests: Arc<
        Mutex<
            HashMap<u64, oneshot::Sender<std::result::Result<Value, ResponseError>>>,
        >,
    >,
    ...
    pub async fn start(cmd: &str, args: &[String]) -> Result<Arc<Self>> {
        ...
        let pending_requests: Arc<
            Mutex<
                HashMap<
                    u64,
                    oneshot::Sender<std::result::Result<Value, ResponseError>>,
                >,
            >,
...
use lsp_types::{DocumentSymbolParams, GotoDefinitionParams};

async fn parse_message<R: AsyncBufReadExt + Unpin>(
    reader: &mut R,
) -> Result<Option<Value>> {
