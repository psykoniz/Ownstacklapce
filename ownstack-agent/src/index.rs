...
impl SemanticIndex {
    ...
    pub async fn index_workspace(&self) -> Result<(), String> {
        ...
        for file in files {
            ...
            for (i, chunk) in chunks.into_iter().enumerate() {
                ...
                self.embeddings.write().unwrap().push(embedding.clone());
    ...
    pub async fn save(&self) -> Result<(), String> {
        ...
        let embs = self.embeddings.read().unwrap();
        ...
        info!("Index saved: {} chunks with embeddings", meta.len());
        Ok(())
    }
    ...
    pub async fn load(&self) -> Result<(), String> {
        ...
        *self.metadata.write().unwrap() = meta;
        ...
        // Rebuild HNSW from persisted embeddings
        let hnsw = Hnsw::new(384, 100000, 16, 200, DistCosine {});
        ...
        if embeddings_path.exists() {
            ...
            let indexed: Vec<(&Vec<f32>, usize)> = embs.iter().enumerate().map(|(i, e)| (e, i)).collect();
            if !indexed.is_empty() {
                hnsw.parallel_insert(&indexed);
            }

            info!("Rebuilt HNSW index from {} persisted embeddings", embs.len());
            *self.embeddings.write().unwrap() = embs;
        } else {
            debug!("No embeddings file found, starting with empty index");
        }
