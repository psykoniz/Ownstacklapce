...
impl PolicyEngine {
    ...
    fn is_blocked(cmd: &str) -> bool {
        let blocked_patterns = [
            ...
            "shutdown", "reboot", "halt", "kill -9 1", "mount ", "umount ",
    ...
    fn needs_confirmation(cmd: &str) -> bool {
        let confirmation_patterns = [
            "git push", "git reset --hard", "npm publish", "cargo publish",
            "docker rm", "docker rmi", "rm -rf ", "curl ", "wget "
        ];
...
mod tests {
    ...
    fn test_blocked_sudo() {
        assert_eq!(PolicyEngine::evaluate("sudo apt update"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("sudo rm file.txt"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("sudo -i"), PolicyDecision::Blocked);
    }

    #[test]
    fn test_blocked_chmod_777() {
        assert_eq!(PolicyEngine::evaluate("chmod 777 /etc/passwd"), PolicyDecision::Blocked);
    }

    #[test]
    fn test_blocked_mkfs() {
        assert_eq!(PolicyEngine::evaluate("mkfs.ext4 /dev/sda"), PolicyDecision::Blocked);
    }

    #[test]
    fn test_blocked_dd() {
        assert_eq!(PolicyEngine::evaluate("dd if=/dev/zero of=/dev/sda"), PolicyDecision::Blocked);
    }

    #[test]
    fn test_blocked_shutdown_reboot_halt() {
        assert_eq!(PolicyEngine::evaluate("shutdown now"), PolicyDecision::Blocked);
    ...
    fn test_blocked_mount_umount() {
        assert_eq!(PolicyEngine::evaluate("mount /dev/sdb1 /mnt"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("umount /mnt"), PolicyDecision::Blocked);
    }

    #[test]
    fn test_blocked_write_to_system_dirs() {
        assert_eq!(PolicyEngine::evaluate("echo hacked > /etc/shadow"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("echo data > /usr/bin/test"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("echo data > /bin/sh"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("echo data > /sbin/init"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("echo data > /var/log/syslog"), PolicyDecision::Blocked);
    }

    // ─── Ask Commands ────────────────────────────────────────────
    #[test]
    fn test_ask_git_push() {
        assert_eq!(PolicyEngine::evaluate("git push origin main"), PolicyDecision::Ask);
        assert_eq!(PolicyEngine::evaluate("git push --force"), PolicyDecision::Ask);
    }

    #[test]
    fn test_ask_git_reset_hard() {
        assert_eq!(PolicyEngine::evaluate("git reset --hard HEAD~1"), PolicyDecision::Ask);
    }
    ...
    fn test_ask_docker_rm() {
        assert_eq!(PolicyEngine::evaluate("docker rm container123"), PolicyDecision::Ask);
        assert_eq!(PolicyEngine::evaluate("docker rmi image:tag"), PolicyDecision::Ask);
    }

    #[test]
    fn test_ask_rm_rf_workspace() {
        assert_eq!(PolicyEngine::evaluate("rm -rf src/"), PolicyDecision::Ask);
        assert_eq!(PolicyEngine::evaluate("rm -rf node_modules"), PolicyDecision::Ask);
    }

    #[test]
    fn test_ask_network() {
        assert_eq!(PolicyEngine::evaluate("curl https://example.com"), PolicyDecision::Ask);
        assert_eq!(PolicyEngine::evaluate("wget https://example.com/file"), PolicyDecision::Ask);
    }
    ...
    fn test_auto_safe_commands() {
        ...
        assert_eq!(PolicyEngine::evaluate("grep -r pattern src/"), PolicyDecision::Auto);
        assert_eq!(PolicyEngine::evaluate("find . -name '*.rs'"), PolicyDecision::Auto);
        assert_eq!(PolicyEngine::evaluate("head -20 README.md"), PolicyDecision::Auto);
        assert_eq!(PolicyEngine::evaluate("tail -f log.txt"), PolicyDecision::Auto);
    }
    ...
    fn test_auto_git_readonly() {
        ...
        assert_eq!(PolicyEngine::evaluate("git commit -m 'test'"), PolicyDecision::Auto);
    }
    ...
    fn test_case_insensitivity() {
        assert_eq!(PolicyEngine::evaluate("SUDO apt update"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("Sudo rm test"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("SHUTDOWN"), PolicyDecision::Blocked);
        assert_eq!(PolicyEngine::evaluate("GIT PUSH origin"), PolicyDecision::Ask);
    }
    ...
    fn test_special_characters() {
        assert_eq!(PolicyEngine::evaluate("echo 'hello world'"), PolicyDecision::Auto);
        assert_eq!(PolicyEngine::evaluate("echo \"test $VAR\""), PolicyDecision::Auto);
        assert_eq!(PolicyEngine::evaluate("cat file\twith\ttabs"), PolicyDecision::Auto);
    }

    #[test]
    fn test_command_in_middle_of_string() {
        // "sudo" embedded in a longer command string
        assert_eq!(PolicyEngine::evaluate("echo sudo test"), PolicyDecision::Blocked);
    }

    #[test]
    fn test_newline_in_command() {
        assert_eq!(PolicyEngine::evaluate("echo hello\nsudo rm"), PolicyDecision::Blocked);
    }
    ...
    fn stress_test_sequential_evaluations() {
        let commands = vec![
            "ls", "cat foo", "rm -rf /", "sudo apt", "git push",
            "cargo build", "grep test", "shutdown", "reboot",
        ];
        for _ in 0..1000 {
            for cmd in &commands {
    ...
    fn stress_test_all_blocked_patterns() {
        let blocked = [
            "rm -rf /", "sudo anything", "chmod 777 file", "mkfs.ext4 x",
            "dd if=/dev/zero", "shutdown -h now", "reboot",
            "halt", "kill -9 1", "mount /dev/sda", "umount /mnt",
            "> /etc/test", "> /usr/test", "> /bin/test", "> /sbin/test", "> /var/test",
        ];
        for cmd in &blocked {
            assert_eq!(
                PolicyEngine::evaluate(cmd), PolicyDecision::Blocked,
